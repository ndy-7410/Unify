...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['ajax_update'])) {
    ...
    $allowed_fields = ['name', 'description', 'status_id', 'priority_id'];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['remove_member'])) {
        ...
        if ($is_owner || $target_user_id == $user_id) {
            ...
            if($target_user_id == $user_id) { header("Location: project.php"); exit; }
        }
        header("Location: task.php?project_id=$project_id");
        exit();
    }
...
$sqlTasks = "SELECT t.task_id, t.name, t.description, t.created_at, t.updated_at, 
...
<head>
    ...
    <style>
        .creator-badge { font-size: 0.75rem; color: #99a2aa; display: block; margin-top: 4px; }
        
        /* --- STYLES PERSONNALISÉS (RESTAURÉS) --- */
        
        /* CHIPS UTILISATEURS */
        .user-chip {
            display: inline-flex; align-items: center; padding: 4px 12px 4px 4px;
            border: 2px solid; border-radius: 50px; background-color: #fff; transition: all 0.2s;
        }
        ...
        /* Propriétaire (Noir) */
        .user-chip.owner { border-color: #000; color: #000; }
        .user-chip.owner .avatar-circle { background-color: #000; color: #fff; }
        /* Collaborateur (Gris) */
        .user-chip.collab { border-color: #dee2e6; color: #6c757d; }
        .user-chip.collab .avatar-circle { background-color: #e9ecef; color: #6c757d; }

        /* Boutons d'action ronds */
        .btn-action-rounded {
            width: 24px; height: 24px; border-radius: 50%; border: none; display: flex; 
            align-items: center; justify-content: center; cursor: pointer; transition: 0.2s;
        }
        .btn-remove { background: #fee2e2; color: #dc2626; margin-left: 8px; }
        .btn-remove:hover { background: #dc2626; color: white; }
        
        /* Bouton Quitter */
        .btn-quit {
            background-color: #dc3545; color: white; border-radius: 20px; font-size: 0.7rem;
            padding: 4px 10px; margin-left: 8px; text-decoration: none; font-weight: 600; border: none;
        }
        .btn-quit:hover { background-color: #b02a37; }

        /* Bouton Ajouter membre */
        .btn-add-member {
            width: 42px; height: 42px; border-radius: 50%; background-color: #212529; color: white;
        ...
        .form-control-transparent:hover, .form-control-transparent:focus {
            background: #f8f9fa; 
            border-color: #dee2e6;
        }
...
<body class="bg-white d-flex flex-column min-vh-100">
            ...
            <div class="col-lg-10 mx-auto">
                ...
                <div class="text-center mt-4">
                    <button class="btn btn-dark px-4 py-2 rounded-pill" id="showCreateTask">
                        <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" fill="currentColor" class="bi bi-plus-lg me-2" viewBox="0 0 16 16"><path fill-rule="evenodd" d="M8 2a.5.5 0 0 1 .5.5v5h5a.5.5 0 0 1 0 1h-5v5a.5.5 0 0 1-1 0v-5h-5a.5.5 0 0 1 0-1h5v-5A.5.5 0 0 1 8 2"/></svg>
                        Nouvelle tâche
                    </button>
                </div>
            </div>
    ...
    <div class="container pb-5 flex-grow-1">
        ...
        <div class="d-flex justify-content-end mb-3 align-items-center small text-muted">
            ...
            <div class="btn-group btn-group-sm">
                <a href="?project_id=<?= $project_id ?>&sort=status&order=ASC" class="btn btn-outline-secondary">Statut</a>
                <a href="?project_id=<?= $project_id ?>&sort=priority&order=DESC" class="btn btn-outline-secondary">Priorité</a>
            </div>
        </div>
                    ...
                    <?php if ($tasks): foreach ($tasks as $task): ?>
                        <tr data-task-id="<?= $task['task_id'] ?>">
                            ...
                            <td class="text-center">
                                <select class="form-select form-select-sm select-minimal task-input" data-field="status_id">
                            ...
                            <td class="text-center">
                                <select class="form-select form-select-sm select-minimal task-input" data-field="priority_id">
                            ...
                            <td class="text-center">
                                <form method="POST" onsubmit="return confirm('Supprimer cette tâche ?');" class="d-inline-block">
                                    <input type="hidden" name="task_id" value="<?= $task['task_id'] ?>">
                                    <button type="submit" name="delete_task" class="btn btn-outline-danger btn-sm border-0">
                                        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-trash" viewBox="0 0 16 16"><path d="M5.5 5.5A.5.5 0 0 1 6 6v6a.5.5 0 0 1-1 0V6a.5.5 0 0 1 .5-.5m2.5 0a.5.5 0 0 1 .5.5v6a.5.5 0 0 1-1 0V6a.5.5 0 0 1 .5-.5m3 .5a.5.5 0 0 0-1 0v6a.5.5 0 0 0 1 0z"/><path d="M14.5 3a1 1 0 0 1-1 1H13v9a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V4h-.5a1 1 0 0 1-1-1V2a1 1 0 0 1 1-1H6a1 1 0 0 1 1-1h2a1 1 0 0 1 1 1h3.5a1 1 0 0 1 1 1zM4.118 4 4 4.059V13a1 1 0 0 0 1 1h6a1 1 0 0 0 1-1V4.059L11.882 4z"/></svg>
                                    </button>
                                </form>
                            </td>
                        </tr>
                    <?php endforeach; else: ?>
                        <tr><td colspan="5" class="text-center text-muted py-5">Aucune tâche.</td></tr>
                    <?php endif; ?>
                </tbody>
            </table>
        </div>
    ...
    <div class="modal fade" id="createTaskModal" tabindex="-1"><div class="modal-dialog modal-dialog-centered"><div class="modal-content shadow border-0"><div class="modal-header border-bottom-0 pb-0"><h5 class="modal-title fw-bold">Nouvelle tâche</h5><button class="btn-close" data-bs-dismiss="modal"></button></div><form method="POST"><div class="modal-body pt-4"><label class="form-label fw-semibold small text-muted">TITRE</label><input type="text" name="name" class="form-control mb-3" required><label class="form-label fw-semibold small text-muted">DESCRIPTION</label><textarea name="description" class="form-control mb-3" rows="3"></textarea><div class="row"><div class="col-6"><label class="form-label fw-semibold small text-muted">STATUT</label><select name="status_id" class="form-select mb-3"><option value="1">⚪ À faire</option><option value="2">🔵 En cours</option><option value="3">🟢 Terminé</option><option value="4">🟣 En test</option><option value="5">🔴 Bloqué</option></select></div><div class="col-6"><label class="form-label fw-semibold small text-muted">PRIORITÉ</label><select name="priority_id" class="form-select mb-3"><option value="1">⚪ Minimum</option><option value="2">🔵 Moyen</option><option value="3">🟠 Important</option><option value="4">🔴 Urgent</option></select></div></div></div><div class="modal-footer border-top-0 pt-0 pb-4"><button class="btn btn-link text-decoration-none text-muted" data-bs-dismiss="modal">Annuler</button><button type="submit" name="create_task" class="btn btn-dark px-4">Créer</button></div></form></div></div></div>
    <div class="modal fade" id="addMemberModal" tabindex="-1"><div class="modal-dialog modal-dialog-centered"><div class="modal-content shadow border-0"><div class="modal-header border-bottom-0 pb-0"><h5 class="modal-title fw-bold">Inviter</h5><button class="btn-close" data-bs-dismiss="modal"></button></div><form method="POST"><div class="modal-body pt-4"><label class="form-label fw-semibold small text-muted">EMAIL</label><input type="email" name="email" class="form-control mb-3" required></div><div class="modal-footer border-top-0 pt-0 pb-4"><button class="btn btn-link text-decoration-none text-muted" data-bs-dismiss="modal">Annuler</button><button type="submit" name="add_member" class="btn btn-dark px-4">Ajouter</button></div></form></div></div></div>
    ...
    <script>
        ...
        document.querySelectorAll('.task-input').forEach(input => {
            input.addEventListener('change', function() { saveData(this); });
        });
